...
public:
               ...
               double sampleRate = 0, int bufferSize = 0)
    {
        ...
        // Snap read pointer to current write position so AudioThru starts from
        // "now" rather than consuming stale samples accumulated while LTC input
        // was running without a consumer.
        sourceInput.load(std::memory_order_acquire)->syncPassthruReadPosition();
...
private:
    ...
    // sourceInput points to a LtcInput owned by BridgeEngine.  Lifetime is
    // guaranteed because ~BridgeEngine() calls stopLtcThru() (which nulls
